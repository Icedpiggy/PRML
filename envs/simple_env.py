...
def test_env(show_bnd=False, randomize=False, debug=False):
	print(f"Init env... (bnd:{'on' if show_bnd else 'off'}, rand:{'on' if randomize else 'off'}, debug:{'on' if debug else 'off'})")
	env = SimpleRodEnv(render=True, verbose=False, debug=debug, show_bnd=show_bnd, randomize=randomize)
	step = 0
