...
class DataCollector:
	...
	def _print_progress(self, step, info):
		if step % 100 == 0:
			st = f"reached:{info['reached_height']} bnd_vio:{info['bnd_vio']}"
	...
	def _collect_loop(self, traj_id):
		...
		while step < self.MAX_STEPS:
			...
			if done:
				self._print_result(info['reached_height'], step, info)
				return traj if info['reached_height'] else None
	...
	def collect_single_trajectory(self):
		...
		print(f"  - Auto-save on successful height reached")
