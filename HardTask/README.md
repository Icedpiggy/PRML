...
这个项目对我来讲还是太难了，我总共采集了 180 条数据作为训练集，20 条作为测试集。尽管模型在训练集上的交叉熵损失能够降至0.02，但训练损失低不代表模型在实际执行时能够完成精确的机械臂操作任务。模型在测试时明显能看出轨迹的模式被模型学习到了，但是甚至没办法定位至刚体棒，会有比较大的误差。

这个以后再改良吧，以我现在初学者的实力确实没办法实现。
